...
#define OCMExpect(invocation)                                                                      \
    ({                                                                                             \
        _OCMSilenceWarnings(                                                                       \
            [OCMMacroState beginExpectMacro]; OCMStubRecorder *recorder = nil; @try {              \
                invocation;                                                                        \
            } @finally { recorder = [OCMMacroState endExpectMacro]; } recorder;);                  \
    })

#define OCMReject(invocation)                                                                      \
    ({                                                                                             \
        _OCMSilenceWarnings(                                                                       \
            [OCMMacroState beginRejectMacro]; OCMStubRecorder *recorder = nil; @try {              \
                invocation;                                                                        \
            } @finally { recorder = [OCMMacroState endRejectMacro]; } recorder;);                  \
    })

#define ClassMethod(invocation)                                                                    \
    _OCMSilenceWarnings([[OCMMacroState globalState] switchToClassMethod]; invocation;);
